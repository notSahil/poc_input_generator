...
### ADR 39: Enterprise HTTPS Reverse Proxy & Application Security Skill (`scripts/nginx_sitetracker.conf`, `scripts/setup_ssl_nginx.sh`, `.agents/skills/security-engineer/`)
- **Decision:** Terminate all public web traffic via an Nginx reverse proxy on port 443 with TLS 1.3 / 1.2, Let's Encrypt SSL (`161-118-182-20.sslip.io`), automated 301 HTTP-to-HTTPS redirect on port 80, security headers (HSTS, nosniff, SAMEORIGIN), Streamlit WebSocket support, and 100 MB upload limits. Bind Streamlit strictly to `127.0.0.1:8501` to isolate it from unencrypted public exposure. Establish a permanent custom agent skill (`.agents/skills/security-engineer/SKILL.md`) enforcing OWASP Top 10 standards, SOQL injection sanitization, CSV formula neutralization, path traversal boundaries, and Bandit SAST linting.
- **Reason:** Upgrades production security posture from cleartext HTTP to encrypted HTTPS with an official trusted SSL certificate, eliminates public exposure of backend application ports, protects against session hijacking, and enforces defensive secure coding standards across all development.
